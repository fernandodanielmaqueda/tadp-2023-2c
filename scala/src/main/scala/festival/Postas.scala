...
trait Posta {
  ...
  def ordenarPorMejor(competidores: List[Competidor]): List[Competidor] =
    competidores.sortWith(elPrimeroEsMejorQueElSegundo)
...
class Pesca(pesoMinimoQueDebeLevantar: kg = 0) extends Posta {
  ...
  def elPrimeroEsMejorQueElSegundo: (Competidor, Competidor) => Boolean =
    _.maximoDeKgDePescadoQuePuedeCargar > _.maximoDeKgDePescadoQuePuedeCargar

  def incremento: Hambre = 5

  def admiteA(competidor: Competidor): Boolean =
    competidor.maximoDeKgDePescadoQuePuedeCargar >= pesoMinimoQueDebeLevantar
...
class Combate(gradoDeBarbaridadMinimo: Barbarosidad) extends Posta {
  ...
  def admiteA(competidor: Competidor): Boolean =
    (competidor.barbarosidad >= gradoDeBarbaridadMinimo) || competidor.tieneUnArmaEquipada
...
class Carrera(kilometrosDeCarrera: km, losParticipantesRequierenMontura: Boolean = false) extends Posta {
  ...
  def admiteA(competidor: Competidor): Boolean =
    !losParticipantesRequierenMontura || competidor.tieneMontura
